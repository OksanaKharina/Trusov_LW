...
#define KEYSIZE 16	 // размер ключа в байтах 
...
typedef enum {
	KeyWords = KEYSIZE / 4,
	NumRounds = 12,  //кол-во раундов
	TableSize = 32   // размер табл = 2 * (NumRounds + 1)
} bogus;
...
void SetKey(unsigned char KeyChar)
{
   ...
   // ввод текста, вывод зашифрованного
void encrypt(WORD *PlainText, WORD *CryptoText)
{
   ...
   // вводит зашифрованный текст, выводит обычный текст
...
void setup() //секретный ключ ввода K[0...KEYSIZE-1] 
{
	//набор констант
...
void main()
{
	...
	WORD CryptoText[2] = { 5555, 9999};
	time_t t0, t1;
	char *keystr = "ToGetACreditForACourse";

	if (sizeof(WORD) != 4)
		printf("RC5 ошибка: WORD имеет %d байтов.\n", sizeof(WORD));

	printf("RC5-32/12/16 пример:\n");
	for (i = 1; i<11; i++)
	{
		...
		//результат, шифрование и дешифрование
		setup();
		encrypt(PlainText1, CryptoText);
		decrypt(CryptoText, PlainText2);

		//результат, проверка на отказ в дешифровании
		printf("\n   текст %.8lX %.8lX  --->  шифротекст %.8lX %.8lX  \n",
